...
export default class RestClient {
    ...
    async get(endPoint: string, queryParams?: Record<string, string | number | boolean>) {
        this._lastResponse = await this.apiContext.get(`.${endPoint}`, { params: queryParams });
        console.log(await this._lastResponse.json());
    }

    async post(endPoint: string, data: any, queryParams?: Record<string, string | number | boolean>) {
        this._lastResponse = await this.apiContext.post(`.${endPoint}`, { data: data, params: queryParams });
    }
